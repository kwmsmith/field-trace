...
def update(T, E, acceptance, improvement, _start, _step, _steps):
    ...
    if _step == 0:
        ...
        stdout.write('%12.2g  %12.2f                      %s            ' % \
            (T, E, time_string(elapsed)))
        stdout.flush()
    else:
        remain = ( _steps - _step ) * ( elapsed / _step )
        stdout.write('\r%12.2g  %12.2f  %7.2f%%  %7.2f%%  %s  %s' % \
...
class Annealer(object):
    ...
    def anneal_nr(self, state, Tmax, temp_steps=100, nover=None, Tfac=0.7, updates=False):

        def update(T, E, acceptance, init=False):
            """Prints the current temperature, energy, acceptance rate,
            improvement rate, elapsed time, and remaining time.
            ...
            if init:
                stdout.write('\n\n Temperature        Energy    Accept\n')
                stdout.write('%12.2g  %12.2f' % (T, E))
                stdout.flush()
            else:
                stdout.write('\r%12.2g  %12.2f  %7d' %  (T, E, acceptance))
                stdout.flush()
        ...
        nover = nover or 10 * len(state)
        nlimit = len(state)

        if updates:
            update(T, E, None, init=True)
            ...
            for k in range(nover):
                ...
                # if updates > 1:
                    # if step // updateWavelength > (step-1) // updateWavelength:
                        # update(T, E, float(accepts)/trials, float(improves)/trials, start, step, steps)
                        # trials, accepts, improves = 0, 0, 0
                if accepts > nlimit:
    ...
    def anneal(self, state, Tmax, Tmin, steps, updates=0):
        ...
        if Tmin <= 0.0:
            raise ValueError('Exponential cooling requires a minimum '
                    'temperature greater than zero; given %f' % Tmin)
        ...
        if updates > 0:
            ...
            update(T, E, None, None, start, step, steps)
